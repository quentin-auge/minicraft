...
- **Villagers**: 24 villagers (8 houses ×3: 2 adults+1 baby, `VILLAGE_RADIUS 28` stone plaza at `villageCenter.y` integrated in `generateWorld` via `computeVillageLayout` before terrain — override `h=villageCenter.y`, `intersectsVillage` skips tunnels/rooms/stairs/trees, `placeVillageHouses` `7×7×5` seeded `hash2` with `|dx|<9&&|dz|<9` veto). Houses enterable (2-wide door facing centre, window opposite, flat roof, palette variance). Villagers use same AABB/gravity as player (`aabbCollidesWorld`/`moveMobAxisX/Z/Y`/`mobPhysicsStep` `GRAVITY 37.44`, moon gravity `*0.5` at `MOON_Y-MOON_R`, `hw 0.27/0.16` `h 1.82/0.98`, `villageBound` clamp, `WALK/2` `2.2` (`WALK 4.4`), flee `WALK×2` `8.8`, liquid float `WATER/LAVA/MOON_WATER` `vel*0.96`). Collisions: monde + joueur (`pushMobsFromPlayer`/`moveAxisX/Z` push via `nearbyMobsFor` grid) + entre eux (`separateMobs`/`mobCollidesOther` via `MOB_GRID 8` `mobGrid` `mobById` `buildMobGrid`, `separate` throttled `1/2` frames). Materials cached (`villagerMatCache`/`villagerHeadMatCache`, `villagerGeo` reused) and spawn snapped to block centers `floor+0.5` with `usedBlocks` no overlap. Trous et eau = murs via `hasMobGround`/`mobBlockedAt` (`>0.02` overlap, `!isSolid y-1` et `WATER/LAVA/MOON_WATER` à `y` ou `y-1` comptent comme vide (eau = trou, évitée comme le vide), `MOVE_X/Z` auto-step `±1` `hasMobGround`/`!aabb`) — `mobProbeFree` et `findVillagePath` (BFS 4-N `0.5`, `pyHint`, `intersectsVillage`/`isInsideAnyHouse` hors cible) l’évitent; `updateMobs` `mobTick` + `addVisit`/`visitGrid` `VISIT_CELL 8` `wanderGoalFor` least-visited `v*10 - d*0.15` `30` tries `2` skip `lastTarget<4`, 12-dir scoring `free*(0.55+0.45*dot)` + BFS `>1.8` ou `probe<0.55`, hysteresis `steerX/Z/Cooldown 0.45/0.35` avoids jitter, failsafe `want==0&&dist>0.6` 8 dirs `free>0.5`, anti-blocage `_stuckT>0.55` `bestF>0.35` `1.5-2.5` `canStand` `hasMobGround y/y±1` + pivot `probe<0.15`. TNT: `handleMobExplosion` grid (`R+1.2`) tombe `0.6s` `smoothstep` (`_fallStart/Target` → `PI/2` + `yOff hw*0.9`), reste `3s` couché puis fade `1s` (clonés) et `isMobStandingOn`/`intersectsMob` via `nearbyMobsFor` protège le bloc sous mob et `breakBlock` refuse `isMobStandingOn||intersectsMob`. `generateWorld`/`removeVillagers` clear `visitGrid`/`mobGrid`/`mobById`. **Panique TNT**: `panicVillagers(cx,cy,cz)` appelée dans `handleMobExplosion` quand `dim=="over"` et l'explosion est dans `(VILLAGE_RADIUS+15)²` du `villageCenter` et `abs(cy-villageCenter.y)<=CLOUD_BASE/2`; tous les villageois `homeId>=0` reçoivent `fleeUntil=now+10+stagger` (`stagger=Math.random()*3`, 0–3 s aléatoire par villageois pour étaler la sortie), fuient vers le CENTRE de leur maison (`house.cx+0.5,house.cz+0.5` via `mode="goOut"`+BFS `target=centre`, `WALK*2`), restent `insideT=10s+stagger` dans la maison si à l'intérieur, puis reviennent à `wander` après expiration (les `insideT`/`fleeUntil` décalés font sortir les villageois en file ~3 s au lieu de tous à la fois, évitant l'embouteillage à la porte); pendant la fuite `mobCollidesOther/separateMobs/pushMobsFromPlayer` ignorent parent/enfant et poussent à `0.22`, `fallen` est annulé pour permettre la fuite; bébés ne suivent pas le parent pendant `fleeUntil`. Nether/End sans village ; `removeVillagers`/`spawnVillagers` sur `buildWorld`/`restoreSave`/`resetDims`/`goToDimension` (regen à chaque load), `mobStats` throttled.
- **Enclos cochons/vaches**: élargissement du village (`VILLAGE_PEN_W 12×VILLAGE_PEN_D 10`, `villagePen:{minX,maxX,minZ,maxZ,cx,cz,vy,gateSide}` placé **avant** les maisons dans `computeVillageLayout` (1200 essais `hash2` `seed+7250/7251`, `+R` dedans, maisons évitent `+2`), `placeVillagePen` étale `GRASS` dedans et clôture `LOG` **1 bloc haut** fermée — seule la physique bloque (`aabbCollidesWorld`/`hasMobGround`, `moveMobAxisX/Z` step désactivé pour `pig`/`cow` sur `LOG` en `vy+1`, trou `AIR` laisse passer). `PIG_COUNT 4` + `COW_COUNT 4` (`pigMat 0xf2aeb2` rose, `cowMat` blanche tachetée, `makePigMesh`/`makeCowMesh` 4 pattes `legBL/BR/FL/FR`, `hw0.45` `h0.92/1.30`, `isInsidePen`/`randomPenPoint`/`wanderGoalForPen` 30 essais `visit*10 - d*0.15`). Même physique que villageois (`villagerHW/H` dispatch par `kind`, `mobBlockedAt`/`mobProbeFree` partagés, eau évitée comme le vide via `hasMobGround` — `WATER/LAVA/MOON_WATER` à `y` ou `y-1` = trou, pas de clamp artificiel — seule la clôture bloque). Grab `ENTER` via `pickMob`/`CARRY_GRAPPLE`, `fallen` `handleMobExplosion` `R+1.2` `0.6s` puis `fleeUntil` (`panicPenMobs` même rayon que villageois `(VILLAGE_RADIUS+15)²` `|dy|<CLOUD_BASE/2`, `stagger 0-3s`, si trou dans clôture `findPenGaps`/`nearestPenGap` → `penGapInside` cherche l'entrée, sinon `randomAroundPenPoint` court autour `WALK*2` `8.8`, `fallen` annulé pour fuir). `updateMobs` branche `pig/cow` : `fleeUntil` hors enclos → `nearestPenGap`/`penGapInside`/`randomAroundPenPoint` (cherche entrée, sinon tour `2s` autour), dedans → `wanderGoalForPen` agitation, sinon `insidePen ? wanderGoalForPen : wanderGoalFor` (`hasMobGround`/`aabb`/`findPenGaps`/`randomAroundPenPoint`), animation 4 pattes `sin*0.65` (`legBL/BR/FL/FR`). `spawnVillagers` spawne cochons/vaches après villageois (`usedBlocks` évite overlap, `gid` continu, total 32), `removeVillagers` les supprime, régénère à chaque `buildWorld`.
