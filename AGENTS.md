...
- **Blocks**: numeric constants + `BLOCK_INFO` (solid/opaque/placeable).
  ...
  Nether portal), LAVA (glowing orange non-solid liquid, unlit `MeshBasicMaterial`
  face materials whose color shimmers in the main loop, `placeable: false`),
  NETHERRACK (dark red Nether rock, in the hotbar after OBSIDIAN,
  `placeable: true`), SOULSAND (dark brown-grey Nether beach block,
  `placeable: false`), GLOWSTONE (hot yellow Nether block rendered unlit via
...
- **The Nether**: a terrifying third dimension under a black sky
  ...
  (`NETHER_LAVA_LEVEL`), winding canyons (`generateNetherRivers`/
  `nearestNetherRiver`) are carved down to the lava so they fill as lava
  rivers, and steep cliffs that drop into the sea get vertical lava streaks
  pouring down their faces (`lava cascades`). Terrain is dark red
  NETHERRACK (placeable, in the hotbar) instead of plain STONE; the top block
  of columns just at the lava line is dark SOULSAND, giving the sea a grim
  black beach; rare columns carry a glowing knotted cluster of GLOWSTONE at
  their summit (plus one floating lump above; the GLOWSTONE texture is drawn
  with hot yellow spots and rendered unlit via `basicFace` so it shines in the
  dark). Rising gold embers (`updateNetherEmbers`/`ensureEmbers`, ~220 additive
  `THREE.Points` spawned only over lava, drifting upward with a sway, fading
  and respawning every ~2–5 s; torn down on leaving the Nether) float up off
  the lava sea all around you. Lava behaves like water: you auto-float to the
  surface (`headInWater` treats LAVA like WATER, buoyancy target 1.8, Space
  swims up), the block-pick raycast skips LAVA, and TNT blasts never destroy
...
- **Save/load**: binary format (`SAVE_MAGIC`, version 4) capturing world
  ...
  pair in v3, the nether dim/seed/blocks added in v4; older v1/v2/v3 saves still load).
