...
- **Pine garlands**: Moon pines only — a finished villager pine gets the fixed glowing 4-strand spirale only when its soil sits in the moon zone (`moonZoneGeo`, filtered at registration and on load, so old saves drop ground garlands); two clockwise strands (blue, red) crossed by two counter-clockwise ones (green, yellow), no key, no HUD, no toast; each strand is resampled at a constant 0.22-block arc step (even spacing top to bottom) floating ~0.8 outside the foliage; every cube is mapped to its nearest trunk/rim block — a cube whose anchor is now AIR vanishes (birth-hole anchors need live support within Chebyshev ≤ 2 instead), so breaking blocks or TNT erases exactly the cubes mapped to them; a twisted 4-strand spire (2 tight turns over 4 blocks, radius shrinking to 0.2, each strand keeping its color, apex-anchored) rises above the summit so it reads clearly; persisted in save v33 (style byte, always spirale, + `plantedPines` xyz + `m`/`e`/`seed`).
...
- **Save/load**: binary format (`SAVE_MAGIC`, version 33) capturing world
