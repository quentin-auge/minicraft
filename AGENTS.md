...
- **Endermen**: teleporting mobs (`kind: "enderman"` in `mobs[]`, `hw` 0.31
  ...
  the grab resolves. The displacement hook cannot latch onto endermen at all —
  firing it while aimed at one does nothing. Carried endermen can be released in the Overworld or the Nether,
...
- **Pigeons**: `PIGEON_COUNT` 50 flying mobs, Overworld only, cruising at
  ...
  `separateMobs` skips pigeons entirely. Mob transparency is per-instance:
  ...
  through all obstacles (`updateTNTTarget` pigeon branch); on contact it kills the bird
