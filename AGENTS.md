...
- **Save/load**: binary format (`SAVE_MAGIC`, version 31) capturing world
  ...
  decode); parrot variant rides the existing `look` byte via `mobLookIndex` (kind code 7 `parrot` added in v31, no size change; pre-v31 saves hold pigeons only and load unchanged); restore is exact-place (`placeMobExact`/`mobRestoreOverlapsPlaced`,
