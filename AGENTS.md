...
- **Pigeons**: `PIGEON_COUNT` 50 flying mobs (spawned Overworld-only, but flight runs in the End and the Nether too — nothing auto-spawns in the Nether, only transported/V-chain birds fly there), cruising at
...
- **Save/load**: binary format (`SAVE_MAGIC`, version 30) capturing world
  ...
  decode); restore is exact-place (`placeMobExact`/`mobRestoreOverlapsPlaced`,
