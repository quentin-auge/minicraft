...
- **TNT**: lighting fuses (HUD fuse sprite), delayed explosions with blocks
  ...
  TNT caught in a blast (or re-broken) detonates immediately. Lit TNT never
  survives a dimension change: `goToDimension`/`restoreSave` run `purgeLiveTNT`
  (fuses, visuals, `tntEta` and the `explosionQueue` are dropped with no blast),
  so a fuse lit before stepping through a portal can't detonate in the wrong
  world. Homing bombs show their live `life` countdown on the fuse sprite
  (`drawFuseSprite` in the `t.pigeon` tick branch, same as placed fuses), so the
  timer never reads frozen. In the End, a
...
- **Portals / dimensions**: portal frames are detected in either orientation —
  ...
  final yaw always faces toward the frame centre (`facePortalFrom`),
  so every Overworld arrival lands facing the portal — while Nether and End
  arrivals land with the portal behind you (`recordDimExit` stores
  `faceAwayFromPortal`; repeat Nether/End arrivals reuse the
  recorded `netherExit`/`endExit` yaw, first visits use the fixed spawn yaw);
  you land just short of the return portal (cooldown +
  ...
  Arriving in the Nether or End lands
  you near the portal you used (`netherExit`/`endExit` recorded by `recordDimExit`
  on departure, resolved via `resolveDimArrival`; first visit falls back to the
  auto-built return portal with your back turned to it
  (`yaw = Math.PI` in the Nether, `yaw = 0` in the End, both facing out into
  the new dimension). The Nether persists
  across entries, like the End. Any portal
...
- **Ender Dragon**: ambient dragon that spawns in the End and flies along a
  ...
  sweeps across the whole platform (tight inner passes, wide laps stretched onto
  the square rim so corners are flown too, per-axis clamped to `±END_PLATFORM_R`,
  the shared square limit every End mob obeys), so
...
- **Pig/cow pen**: village extension (`VILLAGE_PEN_W 14×VILLAGE_PEN_D 12`, `villagePen:{minX,maxX,minZ,maxZ,cx,cz,vy,gateSide}` placed **before** houses in `computeVillageLayout` (1200 tries `hash2` `seed+7250/7251`, `+R` inside, houses avoid `+2`), `placeVillagePen` spreads `GRASS` inside and `LOG` **1 block high** closed fence, plus a 2×2 1-deep corner wading pool 1 block above the ground (`VILLAGE_PEN_POOL_W 2×VILLAGE_PEN_POOL_D 2×VILLAGE_PEN_POOL_DEPTH 1`, `villagePen.pool` flush in the pen corner at `minX+1/minZ+1`, `placeVillagePenPool` sets a STONE floor at `vy` with WATER at `vy+1` plus a STONE L rim on the two inner sides at `vy+1` — the pen's own LOG fence frames the two corner sides, called in `generateWorld` right after `placeVillagePen`; no mob ever spawns inside it — `isInsidePenPool` (half-open `x < maxX+1` bounds so `floor+0.5` centers on the east/south pool cells test inside, same fix applied to `isInsidePen`/`isInsidePool`) skips in villager/pig/cow/wolf spawns (plus re-checked post-loop fallback guards), V-chain (`spawnPigeonChain`) spots and save restores (`restoreOverworldMobs`, looped `+2.5` nudge) as well as in the village-pool wander/spawn/path avoidance, and `penPoolExitTarget` mirrors the water-exit steering for pen targets) — physics blocks via `aabbCollidesWorld`/`hasMobGround` (excludes `LOG` fence top as ground) plus explicit `pigOverlapsFence` (`isPigCow`/`pigOverlapsFence`) in `separateMobs`/`pushMobsFromPlayer`/`moveMobAxisX/Z` that forbids any `pig`/`cow` move onto a solid `LOG` fence cell (corner jam via collisions can no longer push onto rim; wolves still allowed, `AIR` gap lets them through)   and `mobPhysicsStep`/`updateMobs` slide a fence-touching animal back inside
  ...
  within 2 blocks) if they ever end up overlapping a solid fence (so
  `pig`/`cow` never stay on rim, and fence brushes no longer blink them to
  the middle). `PIG_COUNT 4` + `COW_COUNT 4` (`pigMat 0xf2aeb2` pink, `cowMat` white spotted, `makePigMesh`/`makeCowMesh` 4 legs `legBL/BR/FL/FR`, `hw0.32` `h0.92/1.30`, `isInsidePen`/`randomPenPoint`/`wanderGoalForPen` 30 tries `visit*10 - d*0.15`). Same physics as villagers incl. same float in water as player (`mobInWater`/`waterSurfaceForMob` `SWIM_ACCEL 8` `SWIM_BRAKE 2.0` `SWIM_AREA 10` `SWIM_MAX 64` `targetY = surface - h/2`) (`villagerHW/H` dispatched by `kind`, `mobBlockedAt`/`mobProbeFree` shared, water on ground at same level (`WATER`/`LAVA`/`MOON_WATER` at `gy` floor or `py` foot) counts as ground (like wolves) — not avoided, probe/path may go through; `tryMobWaterStep` lets them climb back onto floor at same level smoothly (`JUMP_MIN+2.5` from `fy`/`fy+1` when `mobInWater`), without hesitation, for every kind; only the fence blocks). Grab `ENTER` via `pickMob`/`CARRY_GRAPPLE`, `handleMobExplosion` only panics then `fleeUntil` (`panicPenMobs` same radius as villagers `(VILLAGE_RADIUS+15)²` `|dy|<CLOUD_BASE/2`, `stagger 0-3s`, if gap in fence `findPenGaps`/`nearestPenGap` → `penGapInside` looks for the entrance, otherwise `randomAroundPenPoint` runs around at `WALK*2` `8.8`). Outside the village radius a TNT within 20 blocks horizontally (`dx²+dz²≤400`, `|dy|<12`) triggers 5s (`+0-1s` stagger) panic for nearby pigs/cows that flee away from the blast via `fleePointAway` (7-14 blocks opposite the explosion, `mobProbeFree`/`hasMobGround` validated, `WALK*2`, no pen rally, `_outsideFlee`/`_fleeSrcX/Z` cleared after). `updateMobs` branch `pig/cow`: `fleeUntil` with `_outsideFlee` → `fleePointAway` 7-14 blocks opposite TNT for 5s (`WALK*2`, outside village no pen); otherwise `fleeUntil` outside pen → `nearestPenGap`/`penGapInside`/`randomAroundPenPoint` (look for entrance, otherwise loop `2s` around), inside → `wanderGoalForPen` milling, otherwise `insidePen ? wanderGoalForPen : wanderGoalFor` (`hasMobGround`/`aabb`/`findPenGaps`/`randomAroundPenPoint`), 4-leg animation `sin*0.65` (`legBL/BR/FL/FR`). `spawnVillagers` spawns pigs/cows after villagers (`usedBlocks` avoids overlap, `gid` continuous), `removeVillagers` removes them, regenerated on every `buildWorld`; center-cluster fixed via `wanderGoalForPen` penalties (`8*(1.9-d)`/`6*(1.6-td)`) + repulsion and anti-stuck `wanderGoalForPen` branch and `separateMobs` 3×/frame, pen avgDist `~3–4` vs `~1.9` before, `maxOverlap 0` instrumented.
- **Village pool**: 8×6 swimming pool 2 deep at floor level (`VILLAGE_POOL_W 8×VILLAGE_POOL_D 6`, `VILLAGE_POOL_DEPTH 2`, `villagePool:{minX,maxX,minZ,maxZ,cx,cz,vy}` placed after the pen in `computeVillageLayout` (1200 tries `hash2` `seed+7260/7261`, `+R` inside, avoids pen `+2`, houses avoid it `+2` so no intersection with houses or pen), `placeVillagePool` sets OBSIDIAN floor at `vy-2` with WATER at `vy-1..vy` (surface flush with the stone plaza) plus a 1-block STONE frame ring around the water at `vy-1..vy` (stone walls `+` rim, so every edge water cell touches stone sideways and every bottom-layer cell sits on obsidian) and clears `vy+1..vy+2` to AIR, called in `generateWorld` after houses/pen; `isInsidePool`/`poolExitTarget` exposed in `_test`). All mobs float uniformly at 1/2 height (`MOB_FLOAT_FRAC` 0.5, `mobFloatTargetY`) and share one water-exit jump (`mobWaterExitJump`: live `mobInWater`, `fy`/`fy+1`, `JUMP_MIN+2.5`, used by both `tryMobWaterStep` and `tryWolfStep` for villagers/babies/pigs/cows/wolves/cats without exception, so the ground-level water exit is preserved at the deeper float). Pool wander targets are avoided (`randomVillagePoint`/`wanderGoalFor`/`fleePointAway`/`randomAroundPenPoint` skip `isInsidePool`, `findVillagePath`/`wolfFindPath` route around it, villager/pig/cow spawns avoid it)   and any mob in pool water steers directly to the nearest outside point (`poolExitTarget`, bypassing BFS/avoidance, no extra scans) so exits are quick, smooth and lag-free via the normal jump arc. Bathing is time-boxed: each entry into the village pool (`isMobInPoolWater`) or a moon lake (`isMobInMoonLake`, `MOON_WATER` + `inMoonZone`) rolls a random 1–5 s allowance (`BATH_MIN_T`/`BATH_MAX_T`, transient `m._bathT`/`m._bathMax`, endermen/flyers/held/grapple-frozen excluded) and only then steers out — pool via `poolExitTarget`, moon lakes via `moonLakeExitTarget` (nearest dry `MOON` shore validated as a standing spot at shore-top height with `aabbCollidesWorld`/`hasMobGround` — validating at float height always collides with the 1-block-higher shore and yields no/far targets, latched as `_poolExKind "moon"` until out of `MOON_WATER`), so no ground mob ever stays in either water more than 5 s.
- **Wolves**: `WOLF_COUNT 20` (`WOLF_FUR 0xc8cdd2`, `WOLF_COLLAR_COLORS` 5 — red `0xe53935`, green `0x2ecc40`, blue `0x246bff`, yellow `0xffd600`, turquoise `0x00bfa5` — random per wolf, `makeWolfMesh(fur, collar)` fur grey body 0.92×0.58×1.18, head/snout/ears/tail + U collar (`0.62×0.38×0.08`, `colY 0.69` `colZ 0.64` three `BoxGeometry` bars — bottom/left/right — forming a U open at the top (no top bar, sides terminate cleanly at the top), tucked right under the neck, narrow to sit flush against the head sides with no gap via `MeshStandardMaterial` in the collar color, stored as `collar`/`collarHex` on mob and in `mesh.userData` — but the collar bars are hidden behind `WOLF_SHOW_COLLAR = false`, so wolves render collar-free while all collar code, fields and save bytes are kept) roam the whole map (`villageBound:false` always, forced on spawn and both restore paths; `wanderGoalForWolf` picks map-wide 15–50-block legs validated at live `pos.y`, `wolfFindPath`/`wolfProbeFree` bounded by the map edge instead of the village, `hw0.30` `h0.90`, `WALK/2`; the village (houses included) is never excluded from wander/path). Player-like physics without jump as a **general mob capability `canStep`** (`mobCanStep`/`mobHasGroundFor`/`mobBlockedAtFor`/`mobProbeFreeFor` helpers, `canStep:false` for villagers/pigs/cows, `canStep:true` for wolves and cats) — request said "désactivée pour les loups" but logically wolves must step while villagers cannot, so enabled for wolves; note corrected) — `tryWolfStep`/`wolfMoveAxisX/Z/Y`/`wolfPhysicsStep` reuse `GRAVITY 37.44` moon `*0.5` (`tryWolfStep` now makes them jump `vel.y=JUMP_MIN+2.5` with `onGround=false` and no blocking — `wolfMoveAxisX/Z` returns `false` on jump so they visibly leap onto the marche (`GRAVITY` arc `~1.44` blocks high, lands on `by+1+0.001`), `mobInWater` allows `tryWolfStep`/`mobWaterExitJump` from water at `fy`/`fy+1`, 1 block at a time, `wolfStepUp` kept only for water, `wolfHasMobGround` uses `any` overlap and `wolfMoveAxisY` keeps `onGround` only when `(pos.y%1)<0.05` so 1-block drops fall with `GRAVITY` instead of a `STEP_SPEED` glide) and float in `WATER/LAVA/MOON_WATER` exactly like player (`mobInWater`/`waterSurfaceForMob` `SWIM_ACCEL 8` `SWIM_BRAKE 2.0` `SWIM_AREA 10` `SWIM_MAX 64` `vel.y*=0.3` on entry, `targetY = surface - h/2` waterline at 1/2 height) (water on ground at same level — `WATER`/`LAVA`/`MOON_WATER` at `gy` floor or `py` foot — is not a hole for any mob: `hasMobGround`/`wolfHasMobGround` treat `WATER`/`LAVA`/`MOON_WATER` at `gy` as ground and ignore at `py`, so every mob (villagers, babies, pigs, cows, wolves) may walk through shallow water and climb back onto floor at same level via `tryMobWaterStep`/`tryWolfStep` (`JUMP_MIN+2.5` from `fy`/`fy+1` when in water, `GRAVITY` arc, smooth, no hesitation); `wolfHasMobGround` now uses `any` overlap); `moveMobAxisX/Z` step-up and step-down now gated on `mob.canStep` via `mobHasGroundFor` so villagers/pigs/cows treat holes as walls. TNT panic `panicWolves` at `WALK*2`: outside the village a TNT within 10 blocks 3D (`OUTSIDE_PANIC_DIST`) triggers the standard 3s (`PANIC_TIME`) flee away from the blast via `fleePointAway` like every other outside mob (`_outsideFlee`); a blast inside the village square panics only wolves inside the square and they **leave the village** like pigeons on the ground — `wolfLeaveTarget` (10 tries x 20-35 blocks away from the blast, rejecting targets still inside the square via `villageSqContains`, validated with `wolfProbeFree`/`wolfHasMobGround`, `fleePointAway` fallback; shared away-from-blast direction helper `panicLeaveDir` also used by `panicPigeonLeaveTarget`) at `WALK*2` for `VILLAGE_PANIC_TIME` (5s, `_villageFlee` re-picks leave-targets while it lasts, saved as outside-flee so reloads resume fleeing); chain-sever `panicSingleMob` converts wolves freed in the square the same way. Grab `ENTER` via `pickMob` (ray-AABB, `villagerHW/H` dispatch). Spawned after pigs/cows in `spawnVillagers` on firm flat ground outside the village (`wolfFlatSpot`: top-solid 3x3 all-equal height, natural ground, 2 air above, village bounds +2 excluded, 20-block spread, total 61 = 24 villagers+8 pen+20 wolves+1 golem+8 cats), `removeVillagers`/`buildWorld` regen.
- **Iron golem**: `GOLEM_COUNT 1` (`makeIronGolemMesh` light-grey body with moss patches, brow band, protruding nose and red eyes, long shoulder-hung arms, `hw0.6` `h3.6` = 2x player). Wanders the village like a villager (`villageBound:true`, `homeId:-1` so it never goes home/inside, `wanderGoalFor` at `WALK/2`, `canStep:false`, spawns outside houses/pen/pool via `spawnVillagers` top-up, `mobProbeFree`/`mobBlockedAt` height-aware via `GOLEM_HW/HH`). ENTER-grab refused, displacement ride allowed, chain lead-only (`linkChain` refuses it as child, never in V-chains), never panics (`panicGeneric`/`panicSingleMob` skip), persisted as mob kind code 6 in save v29.
- **Cats**: `CAT_COUNT 8` — 1 per baby villager (`kind: "cat"`, `hw0.16` `h0.98` = baby AABB, `CAT_HW/HH`). `makeCatMesh(robe)` boxy quadruped (body/head/muzzle/nose/eyes/ears/upright tail with light tip/3 back stripes/4 legs, `userData` legs+`catVar`, leg anim `sin*0.65` like wolves); 3 robes via `look` (`CAT_ROBES` ginger `0xdd8a3c`/black/white, weighted `[2,1,1]` by `pickCatRobe`, ginger default). Spawned in `spawnVillagers` after the golem: each baby without a cat gets one (`homeId` = baby's house, `parentId` = baby id, `villageBound:true`, `canStep:true`), placed on the baby cell or a nearby validated village spot (`usedBlocks`, house/pool/pen exclusions, `groundYForMob`); top-up heals orphans (old saves, V-chains), `removeVillagers` clears them. Strict 1-1 follow of their baby (`catParentFor`/`catTrailSpot` mirror the baby helpers — same dim, `|dy|<1`, `FOLLOW_TRAIL_D` 1.125 behind parent yaw, `follow` past `FOLLOW_ENGAGE_D` 1.5 holding until under `FOLLOW_HOLD_D` 0.9 at 1.3x `WALK/2`, mill ±`FOLLOW_MILL_R` 0.75, same house-door split, preempted by `fleeUntil`; parent/enfant pairs exempt from separation via `mobBondedPair`, also covering head-on and push guards; `restoreInitialTarget` resumes the trail on load). Follow steering is strict chain-like within `FOLLOW_LEASH_D` 12 (`followParentOf`/`isStrictFollower`: no BFS, no 12-dir deflection, no repulsion steering, no head-on sidestep, anti-stuck resets without retargeting — direction changes only via collision slide/step-up; beyond the leash normal steering resumes for long chases; panic/door-split/pool-exits preempt). Jump like wolves: `isJumpingKind` covers wolf+cat so the whole `canStep` path (`wolfPhysicsStep`, `wolfFindPath` with house/pool/pen avoidance, `wolfProbeFree`, `_chainStep` grant/suppress) applies; village wander uses `wanderGoalFor` (not map-wide wolf legs) and cats join `isHeadonWalker`. TNT panic rallies home like villagers (`panicCats` 8s + 0–3s stagger, `goOut`/`inside` at the baby's house, `resumeMobPanic` same branch; outside the square the standard 10-block 3s `fleePointAway`). Standard interactions: ENTER-grabbable, chains `behind` (in `CHAIN_SPAWN_KINDS` + `spawnChainMob` sizes for V), portal trips with no special-casing, persisted as mob kind code 8 with the robe in `look` in save v32 (pre-v32 saves top-up fresh).
- **Mob carry (ENTER)**: press `Enter`/`NumpadEnter` to fire a red grapple (`CARRY_GRAPPLE_CUBES` 2600 `#870000` `0x870000` cube chain `opacity 0.3` + `carryGrappleHead` `0x5a0000` `transparent` `opacity 0.3`) — grab hook flies to mob center (`pos+h/2`, clamped to exact mid-body `(0, h/2, 0)` for endermen so tall grabs never catch the head) at `MOB_GRAPPLE_THROW = GRAPPLE_THROW*1.25 =87.5` horiz. (`175` vert. with the same `grappleVertBoost` ×2 as displacement, `isVert=|dy|>2*|dx|`), homing: `updateCarryGrapple` each tick sets `carryGrappleTarget=mob.pos+h/2` and moves `carryGrappleHookPos` toward it at `MOB_GRAPPLE_THROW*tb` (hit point `getMobHitOffset`, smooth `min(step,dist)` no last-moment snap, rope follows `hookPos`) (passes through walls, ignores collision), `carryGrappleCubes` rope follows `hookPos` (not `lerp`) and is rendered at `30%` opacity both ways (`carryGrappleCubeMat`/`carryGrappleHead.material` `opacity 0.3`/`transparent true`); mob **keeps moving** during grab chase (not frozen — `startCarryGrabGrapple` does not touch `vel/path`, `isMobFrozenByGrapple` returns `carryGrapplePulling` only for grab so `updateMobs`/`separateMobs`/`pushMobsFromPlayer`/`moveAxisX/Z` keep wandering until contact, then freeze) and for release the mob is frozen from the start (`isMobFrozenByGrapple` true when `mode==="release"` and `Active||Pulling||Retracting`, so `mob.pos` rides `carryGrappleHookPos` at `MOB_GRAPPLE_THROW*tb` with no wander, `hitR=hw+0.35`); no issue if the mob goes behind a wall, the hook passes through; as soon as it hits it pulls back **with the mob** at `MOB_GRAPPLE_RETRACT = MOB_GRAPPLE_THROW*1.25 =109.4` (rope follows `mob.pos+h/2` back to `eye+0.3`, `carryGrappleHookPos` locked to mob so you see the mob ride the hook tip at `alpha=1`, no fading — dead `alpha`/`pullDist`/`Fly` code removed) held   (`carryMob`, `mode="carried"`). ENTER grab refuses the dragon and the iron golem (`startCarryGrabGrapple` shows a toast; the displacement hook can still latch/tow a golem). If the grabbed mob is also the displacement grapple's
  ...
  any dimension instead of tripping the grapple dim-mismatch guard). Empty retracts (with or without mob) always use `GRAPPLE_RETRACT` with no timeout. After any grapple you must release `Enter` and press again (`e.repeat` ignored).   `pickMob` ray-AABB unlimited range (used with `blockDist` max for `fireGrapple`). While held, `updateCarry` moves mob to `eye+fwd*CARRY_DIST(1.2)-CARRY_DOWN(0.40)-h/2` with `playerArms` visible. In the End a release aimed outside the platform square (`endBlockOutsidePlatform`, `±END_PLATFORM_R`) is refused silently for all mobs — the mob stays in hand. A release aimed at an active portal fill sends the mob through the portal instead of dropping it (`mobPortalTx`): the release hook stops at a hover point `MOB_PORTAL_TX_STANDOFF` (2.0) blocks short of the fill center along the eye direction (nudged out of rock), so the mob never sits inside the fill and stays fully visible, then freezes and shrinks in place over 0.3 s (`MOB_PORTAL_TX_TIME`), then lands in the target dimension within 5 blocks of that dimension's portal frame (`mobPortalAnchorWin` + `portalFrameBBox`, upright and flat frames alike — ground mobs take a random standable cell at Chebyshev ≤5 of the frame box within ±5 of its height span, pigeons clamp to the same XZ range; closest-standable fallback, then the old below-spot/raw-spot safety nets), as an unbound cache entry (`mobPortalCacheFor` + village top-up when the Overworld was never populated), so portal trips and saves find it there with no save-format change. Portal-sent entries carry a `portalSent` marker (persisted in save files via spare `panicFlags` bit 2, no version bump; live panic logic only reads bits 0–1), consumed on the first cache→live restore (`restoreOverworldMobs`/`restoreDimMobs`) into a `PORTAL_ARRIVAL_FREEZE` (2 s) stand-still (`_frozenUntil`, enforced in `updateMobs` before all kind dispatch plus `separateMobs`/`pushMobsFromPlayer` mover skips, pause-shifted by `shiftPausedTimers`, re-emitted by snapshots while unexpired) — so a mob you sent stands still for 2 s the first time you see it in the new dimension while every other mob moves normally; grabbing/chaining it during the freeze still works (deliberate player action wins). No mob ever stays inside a portal fill: `updateMobs` rebuilds a live fill-cell set per frame (`rebuildLiveFillCells`) and any same-dimension mob whose body overlaps a fill for over `FILL_SLIDE_TRIGGER_T` (0.2 s, so cruising pigeons pass through untouched) starts a two-phase ejection (`startFillSlide`): first to the nearest free non-fill neighbor cell — the fill's surface — then to the nearest standable ground (`fillSlideGroundSpot`, r≤10, body fill-free, `groundYDown` fallback), sliding at `FILL_SLIDE_SPEED` (12) with AI/physics skipped until arrival; sliding mobs are skipped as movers in `separateMobs`/`pushMobsFromPlayer` but stay obstacles, and grabbing/chaining one discards the slide. Only active portals count: `rebuildLiveFillCells` skips fills whose frame is broken (`portalFillValid`) or sealed (`end` + `!endCleared`), the arrival ban (`mobPortalDestCells`) is empty while the End is sealed, and a dormant fill aim degrades to a normal drop instead of a portal send — render distance/frustum culls stay visual-only, so far-but-lit portals still rescue. Slides clear the whole portal, not just the fill: the overlapped cells resolve to their frame boxes (`liveFillWin` + `portalFrameBBox`, upright and flat alike) and the final ground must sit at Chebyshev ≥1 outside every touched box, so flat 1-thick rings are fully cleared; the search runs r≤16 with nearest-first ±8 vertical hints, so floating or buried portals resolve to the nearest y up or down (`groundYDown` fallback gated on the same frame clearance). Plumbing note: `buildReturnPortal`/`buildNetherPortal` take an optional `skipMesh` so mob-send ensure-generation never poisons the dim-less chunk cache.
- **Mob chains**: with a mob carried, pressing ENTER while aiming at any mob (any kind, chained or solo, same dimension — solo endermen refused like the grapple, and `linkChain` refuses an Enderman root as carrier so no Enderman can ever become a chain lead, and refuses `iron_golem` as child so a golem is ever only a chain lead (`chainAttachModeFor` forces `behind` for golem targets)) attaches the carried mob by flight class (`isFlyingKind`: pigeons, parrots and dragons fly, everything else is grounded; mode picked by `chainAttachModeFor` — every chain/TNT/grapple condition that is about flight keys on it, while bird-only systems (flocking, perch/coop, takeoff, sizes, saves) stay literal): a flying mob onto a grounded lead (or solo mob) goes in front — new lead via `prependChainLead` (collision-free placement via `seatNewLeadNear`; takes off and airlifts, including a fresh 2-chain onto a solo mob) — while every other pairing goes behind the aimed mob (`insertChainBehind`: same seating/rollback/snap pattern, position 2 behind a lead, insert behind a member, true append on a tail; a perched pigeon carrier takes off). The dragon always stays root as a corollary (it is flying, so everything links behind it); aiming at anything else releases to the ground as before. While the player is latched onto the carrier itself (displacement-grapple follow on any mob — pigeons, dragons and regular ground mobs like villagers, wolves, pigs and cows alike, `playerInChain` true for every latched mob), attaching chains the mob behind the **player** instead of the bird: the player joins the chain as a sentinel member (`PLAYER_CHAIN_ID "player"`, a `playerChainAvatar` pseudo-mob with live `pos`/`vel` getters, `playerInChain`), so the chain reads pigeon → player → mob — the carried mob always lands adjacent to the player on the aimed side: aiming at the ride inserts it right behind the ride (`playerAhead` → `insertChainAheadOfPlayer`: linked behind the ride, then the ride switches to it so the player follows it) while aiming at the direct follower inserts it right behind the player (`playerBehind` → `insertChainBehindPlayer`: seated near the player, spliced ahead of the old follower); aims anywhere else in the chain insert immediately behind the aimed mob, and the flying-onto-grounded-lead prepend exception still wins everywhere (new lead + takeoff, latch untouched). `chainAttachTarget` returns the avatar for ride aims and the aimed mob otherwise, the attach hook flies to the target, `linkChain` accepts the avatar as carrier, `chainRootOf`/`chainTailOf` walk through it, `updateChains` tows the mob one link behind the player at the latch target's trail distance (`chainFollowDist`: 2.5 bird / 8 dragon) with the rope rendered from the player, firing the displacement grapple at another member of your own chain re-latches the player right after it (old follower spliced back onto the old ride first, then the same insert; fresh `PLAYER`-carrier links get a `CHAIN_PLAYER_SNAP_T` (2 s) convergence grace with leash/split severs frozen and unbounded 1x snap, so flying/dragon inserts keep their tail exactly like ground inserts; the insert fires at follow handoff (`dist <= followR`), not at hook contact, so no V cord forms while the player is still in transit), and any unlatch (release, retract, `pruneChains` backstop) re-links the whole sub-chain onto the latched bird/dragon (`spliceChainLink` from the per-link `playerFrontId`, appended after any pre-existing children) with the new link stretched up to 2x and gliding to one link while fresh; the player entry never reaches saves (its id is absent from the mob index, so serialization skips it). Attaching to a perched/landing pigeon (roof, cloud, tree) works too: the carried mob arrives centered on the sitting bird, which buries tall mobs in the surface and used to make `linkChain` refuse — now a colliding arrival is re-seated standing on the carrier's surface (`seatChainChildOnCarrierSurface`, nearby offsets tried) before giving up, a successful attach makes a perched/`toPerch` carrier take off (`birdTakeoff`, like a displacement latch does), and carriers never roll perch legs or perch hops while carrying (`birdNextLeg`/`birdTakeoff` skip when `chainChild.has`), so chains stay airborne. Each persistent link is a regular displacement hook (brown `0x8a6d3b` rope + `0x4a3a1e` head per link, opaque — dragon-rooted links use the dragon-eye purple instead (`syncChainLinkColors`), holding single-file trail slots behind its carrier (`chainFollowDist`: 2.5 bird / 8 dragon, base exactly `followDist`, `updateChains`). Every link anchors mid-body to mid-body on both ends (`chainMidY`: `pos.y + h*0.5`, rope included via `renderChainLink`). Flying-led links (flying root) hold a straight formation off the lead's live flight axis: one smoothed axis per flying root (`_chAx`, fast `dt*8` lerp of the velocity heading, climb clamped ±0.6, held while hovering) with a smoothed turn rate (`_chAxV`) so slots swing without lagging into stretch, and each follower steers toward its own carrier's slot one link behind on that axis (`chainFollowDist`, mid-body frame via `chainMidY` so mixed-kind trails fly level instead of stair-stepping) with `vel = carrier velocity + turn-rate swing + 5 × offset` (`CHAIN_TOW_KP`), capped at `leadSpd + 8` (`chainLeadSpeedOf` on the root lead, so dragon surge transmits directly) — first-order convergence through `chainMoveAxis`, no springs, no accordion, no orbit and no `^` on climb-then-dive (slots sit on the current axis by construction, so the chain is straight at all times and only bends transiently inside hard reversals). Links update root-to-tail (`chainDepthOfId` order) so each follower sees its carrier's fresh position. A link stretched past 1.5x but within `CHAIN_FLY_SNAP_MAX` (2.2x) snaps its feet onto its slot on the first free frame (splices land at 1x without giant pops, velocity re-seeded from the slot motion); a link stretched past `CHAIN_FLY_LEASH` (2.5x) for over `CHAIN_FLY_LEASH_T` (1 s) — e.g. a follower wedged on a cloud while the lead flies on — is cut loose via `freeChainRoot` instead of hanging forever, so ropes never span the map and freed mobs never teleport; `flySplitT` sever and jumping hop-assist are kept. Chained followers smoothly rotate to face their travel direction (flyers through a two-stage yaw smoother, ground followers eased at 10/s toward the velocity heading with a 0.08 rad deadband below 2 b/s for tall, h >= 2, bodies so towed endermen never wobble on tow noise). Past `CHAIN_FLY_SPLIT_DY` (8) vertical for over a second (`link.flySplitT`, read before the recoil so a snagged follower can't hide behind the snap) the link is cut via `freeChainRoot`, same sub-chain semantics as the ground split. Hole threading (`chainThreadRide`, both tow branches): a follower stretched past 1.5x whose direct segment to the carrier is blocked by rock rides the carrier's crumb trail kinematically instead of pressing the wall — 3D `chainSlideToward` steps toward successive crumbs (which inherit the lead's funnel centring) with leg physics suspended — while `farT`/`flySplitT`/`strainT` freeze, so the chain survives the hole instead of severing; the ride gives up after `CHAIN_THREAD_MAX_T` 4 s (normal rules resume) and never engages in open sky (direct segment free), so leash/split severs there are untouched. Ground-led links are walked, not dragged (`updateChainGroundLink`, reached whenever the chain root is grounded — the tow routes on root kind, so every member of a bird/dragon-led chain flies in formation while a bird chained behind a walking lead hover-walks via `floats`): each follower steers toward its slot (smoothed `towDir` + `chainTrailTarget` crumb + `towPos`, same 0.4-spaced crumbs capped at 80 with teleport-clear, straight tow as fallback on short trails) with its own legs (`mobPhysicsStep`/`wolfPhysicsStep`, so gravity, buoyancy, holes-as-walls, pen fence and mob collision all apply) at `WALK/2`, hustling at `WALK×2` when strained, with the tow spring kept as a strained-only assist (same stiff/damp, `taut` boost kept) plus tow-target feedforward so followers track the swinging slot through turns instead of cutting the corner; link distance is held in a 0.5x–1.5x band around base by a collision-aware solver (`chainSlideToward`, 0.5 sub-steps, never through rock — no ghost anywhere on the ground path): past 1.5x the follower is pulled in toward the carrier, below 0.5x it is pushed back out along the link axis, and the carrier itself is never yanked. The band/strain/leash is measured mid-body to mid-body (`chainLinkDelta`/`chainMidY`, not top-to-middle), so a tall follower reads no height offset as strain — a 2.7-block enderman tows with the same calm band as a villager — while the vertical split keys on the carrier-vs-follower `dy` so a roof-top carrier still severs its ground follower. Grounded carriers get horizontal-only slides that never ratchet followers up walls (vertical splits are the split-sever's job); while `carrierAirT` (airborne over 0.5 s) the slides stay 3D and the speed cap lifts to bird pace so airlifted followers climb with the flight. Past the 2x leash neither endpoint may recede (a velocity filter strips both separating velocity components) and a true root (unchained, non-player, ground) is braked to a stop until the follower closes — stretch never severs, it waits. Strained followers hop 1-block lips with headroom (`JUMP_MIN + 2.5`, 0.8 s cooldown; wolves step on their own via `canStep`). Flying followers behind a grounded carrier float-walk through the same state machine (`floats`: identical band/leash/strain handling with the mesh pitch kept, but hover locomotion instead of legs — critically-damped toward slot height, integrated via `chainMoveAxis`, no gravity, no hop, 1-block lips always glided over, `_chainStep` skipped; in water the hover yields to the shared half-height buoyancy (`mobInWater`/`waterSurfaceForMob`/`mobFloatTargetY`, damped entry, `SWIM_MAX` cap) and a blocked horizontal push onto a same-level shore fires the shared `mobWaterExitJump` with a short ballistic arc (`_chainJumpT`, cleared in `resumeChainedMob`), so chained birds bob and climb out exactly like ground mobs), so a bird middle hovers ~0.9 above the carrier's feet (smoothed hover floor + 0.12 deadband, so short carriers like babies/pigs/wolves never drag it into the ground) and never disturbs the walking chain: flying followers track the direct tow slot instead of the crumb trail, driven by a low-gain spring plus smoothed carrier-velocity feedforward; the exact-1x snap fires only while the link is fresh (`snapT` 0.4 s after link/splice); band pull-in/push-out teleports and crumb threading are skipped for flyers (gentle strain spring, leash velocity filter and leash clamp instead); a stretched follower whose segment to the carrier is blocked loiters in place until back in band (`link.loiter`, so no frantic wall-circling when the carrier ducks indoors); velocity changes are slew-limited to 12 blocks/s²; heading runs through a two-stage yaw smoother (`link.faceYaw`). Level splits break the link instead of climbing: a follower strained out-of-band with no
...
flight (slides stay 3D and the speed cap lifts to bird pace while `carrierAirT` while
...
or phase. Step-up is root-gated by jumping leadership: when the chain root is a jumping kind (`isJumpingKind`, the wolf and the cat — checked via `chainJumpLed`), every ground member gains full jumping legs (`canStep` plus a `_chainStep` tag — steps, drops, holes and even the pen-fence hop follow the lead wherever it goes, since the jumping path has no fence logic); when the root is anything else, chained jumping kinds lose their step-up (suppressed under the same tag) and the whole chain walks step-free. The tag re-evaluates per frame (root swaps apply instantly) and is always restored kind-correctly on unlink (`resumeChainedMob`, `chainTakeForCarry`; saves re-derive it because `canStep` is kind-derived at spawn/restore). Chains are single-child lists rooted at a pigeon, a ground mob (Overworld or Nether) or the dragon (End), uncapped, persisted per dimension (suspended on portal trips via `suspendLiveDim` snapshots and relinked on return, so every dimension resumes exactly where you left it; all dims persist across save/resume in v16 — Nether mobs are transport-only, saved as the live `dim=nether` population via `snapshotMobsForDim`). `linkChain`/`updateChains`/`chainAttachTarget`/aimed-TNT (`aimedPigeon`/`tntChainAimMob`/`tntFizzleAim`/`igniteTNT`) all accept the Nether, `spawnPigeonChain` (V) works there too, and `killChainMob`/`respawnChainMob` preserve the victim's dim with a dim-aware band clamp. ENTER-grabbing any chain member takes it to hand while the chain splices around it — the mob behind re-links to the mob in front (`chainTakeForCarry` + `spliceChainLink`, also driven by the `updateChains` carrier-held backstop), the new link starting at up to 2x length (clamped 1–2, measured at splice) and snapping back to exactly one link on the first free frame via the 1x projection; when the splice finds no seat the downstream still survives as its own sub-chain (`freeChainRoot`) instead of dropping; every chain lead keeps a 45° keep-away cone around its direct follower (`chainLiveFollower`/`chainLeadConeDeflect`, `CHAIN_LEAD_CONE`): ground leads deflect steering to the nearest cone edge (both edges world-blocked falls back to the original heading, never a freeze), pigeon leads deflect leg targets and skip arcs while facing the follower, dragon laps resample waypoints out of the cone — so leads never turn back through their chain; grabbing the lead breaks only its link to the next — the downstream keeps its links with the next mob resumed as the new root (same for a dead front, and a grabbed tail only ever cuts its own link); TNT removals sever instead of splicing (`unchainMob` → `severChainMob`, `killChainMob` inline — both keep the downstream sub-chain via `freeChainRoot`, except a downstream starting with an Enderman: `freeChainRoot` frees the Enderman solo and roots the surviving tail behind it, so TNT never leaves an Enderman as chain lead); Displacement grapple aimed at a chain member inserts the player right after it (`fireGrapple` → pending insert resolved on hook arrival via `playerInsertCutAndLink`, `latchPlayerInMiddle` for the instant path; the aimed mob's previous follower re-links behind the player, solo endermen still refused, chained ones latch normally) and follows at chain length; latching a solo ground mob tows the player behind it at the same trail distance, with a 1-block step-up on the tow (a velocity hop when the ride is a wolf); releasing any mob-follow inherits the mob's velocity with no fling boost (`flingActive = false`), then gravity. In the End, chained/carrier non-dragon mobs are TNT-targetable under the same aim/lock rules as pigeons (`aimedPigeon` + the ignite path accept them there; solo endermen stay refused). A chained enderman works like any other chain member for everything (grab, grapple latch/tow/release, TNT, fall) and reverts to special behaviour the instant it unlinks. Chained endermen never teleport: `updateEndermen`/`updateMobs` skip chained members entirely, so stare state freezes and no blink fires until unlink. TNT only ever damages leaders: a dragon hit deals full damage without touching links (stuck blasts route to the homing point-blank queue entry so `damageDragon` + tinted burst fire; death drops the chain via `removeDragon` with freed endermen falling to the platform). Any airborne pigeon hit — solo, lead, middle or tail — explodes and respawns (`killPigeon` out of view for solos, `killChainMob`/`respawnChainMob` at origin for chained ones, downstream falling); a homing TNT hitting a grounded chained victim (any chained ground mob, a chained enderman, or a chained pigeon on the ground — perched, sitting or otherwise grounded via `isGroundedChainVictim`) bursts harmlessly with no terrain or mob damage and just severs both adjacent links (`severGroundedChainVictim`: `severChainMob` + resume in place with `villageBound`/`penBound` cleared, so they mill locally around the drop spot via `wanderNear` instead of hiking back to the village/pen — no kill/respawn/damage; freed endermen fall with real gravity (`falling`/`fallV` in `updateEnderman`, set in `resumeChainedMob` so every unlink path grounds them, same `GRAVITY` in every dimension, landing on a downward-only `groundYDown` scan and floating in water instead) instead of hovering down, then resume staring once landed. TNT severs the chain at the victim: `severChainMob` cuts only the two adjacent links — the upstream ends at the surviving predecessor, and the downstream always survives as its own sub-chain (`freeChainRoot`: parent entry + rope removed, new root resumed with bounds cleared so it mills locally, links below kept; held-mob edge disposed without resume) — it keeps flying behind a flying root and walks behind a grounded one via the normal ground links (flying tails ride the spring tow); a hit tail just unlinks. When a chain lead is ENTER-grabbed, only its link to the next mob is cut — the downstream keeps its links with the next mob resumed as the new root (bounds cleared, mills locally). Flying victims explode and respawn (`killChainMob`/`respawnChainMob` at origin); grounded victims (and endermen) take no damage — the homing bomb bursts harmlessly with no terrain damage and the victim falls in place with bounds cleared, milling locally instead of hiking back. Aimed TNT clicks on a grounded chain victim never fall back to placing a block: when the fire is refused (occluded aim, mid-volley lock, burst cap) the click is swallowed via `tntChainAimMob`, so no TNT block ever appears on the ground. The hold latches as non-placing (`leftNoPlace` set when the press fires or aims at a chain victim), so the paint and bridge phases cannot lay TNT blocks while the bombing run lasts. ENTER-grab keeps the splice-around behavior via `chainTakeForCarry`. `groundYDown` exists because the shared `groundYForMob` ±24 window misses ground more than 24 below and falls back to the column top (moon/clouds) — which used to rocket freed or staring endermen skyward; `endermanSurfaceY` uses it too), clears the TNT lock, fizzles the rest of the in-flight volley into harmless bursts, and detaches a following displacement grapple. A cooped carrier pigeon keeps its chain: the lead mills indoors via the tunnel planner while followers tow with the normal collision-aware slides, and only an over-stretched link severs singly via the leash/split rules instead of dissolving the whole chain. Verified headless: 33/33 overworld (link/tow/ropes, regrab split, kill+origin respawn, ENTER attach/regrab, middle insert, no `PAGEERROR`) + 14/14 dragon links (8.0/2.5, no-teleport, damage-safe, death break). Player-in-chain and splice verified headless: attach lands adjacent to the player with tow one link behind the player and the rope from the player, unlatch and middle-grab splices stretch toward 2x then snap to 1x, root-grab keeps dropping downstream, End dragon link tows at 8.0 and splices to the dragon on unlatch, save round-trips a player-held link back into a direct pair with the player falling, no `PAGEERROR`. Takeover/prepend/insert (TNT severing both adjacent links with flying-respawn/grounded-fall victims, any-mob ground chains, flying-lead prepend with takeoff, universal insert-behind, player-adjacent attach with ride switch) plus the flight-class rule (flying onto a grounded lead goes in front, all other pairings insert behind, dragon always root as a corollary) plus sliding around rock (no phasing anywhere — every tow moves via `chainMoveAxis`/`chainSlideToward` only) and jumping hop-assist (`chainHasJumping`) — every link is projected to exactly one link length on the first free frame (mid-anchored slot snap on flying-led links — the slot is the child-feet position whose middle sits exactly one link behind the carrier middle — horizontal snap on ground-led links; all movers stay feet-based so feet stay on terrain), so splices land at 1x with no ease-out and stretch past 1.5x only while rock blocks the snap (bounded by the 2x leash). Ground band/leash/split-sever verified headless: roof/ground split severs into live sub-chains while level links survive, pigeon airlifts ride out the flight under 2x and resume in band, re-splices snap to exactly 1x, circular-stress holds 1x with zero in-solid, no `PAGEERROR`. Pressing V spawns a random pigeon-led chain of 3–8 mobs on the ground where the crosshair points, in the Overworld, the End and the Nether (`spawnPigeonChain`) — a pigeon lead plus random villagers/pigs/cows/wolves/cats (`spawnChainMob`, unbound, collision-free row validated with `groundYDown`/`aabbCollidesWorld`/`hasMobGround`, the lead built at true ground level, never lifted into the flight band; the row anchors at the `pickBlock` aim cell (face-adjusted, receding from the player) and falls back to a random spot near the player when aiming at the sky) linked with `linkChain`, the lead standing still on the ground in `sit` mode (held by `updatePigeon`), spawned mobs take the current dimension (`spawnChainMob` stamps live `dim`, so End chains fly and wander there; pigeons fly in the End too, still frozen in the Nether), End chains are purged on exit (`removeEndEntities` drops dim-end leftovers like `removeVillagers` does for overworld ones, never reaching saves) — which takes off after 2 s (`setTimeout` → `pigeonTakeoff`, skipped if the lead no longer carries the chain).
- **Endermen**: teleporting mobs (`kind: "enderman"` in `mobs[]`, `hw` 0.31
  ...
  - Classic "don't stare" behaviour: holding the crosshair on the enderman's
    ...
    In the End every destination is clamped to the square platform limit
    (`±END_PLATFORM_R` via `endBlockOutsidePlatform`: `endermanPickSpotEnd`/`endermanPickSpot`
  ...
  is special again (stare, teleport, no latch). An enderman chained under the
...
- **Birds (pigeons + parrots)**: `BIRD_COUNT` 50 flying mobs (spawned Overworld-only, but flight runs in the End and the Nether too — nothing auto-spawns in the Nether, only transported/V-chain birds fly there), cruising at
  `BIRD_SPEED` (`WALK*2` = 8.8) inside the day-sky band Y 50–235
  (`BIRD_MIN_Y`/`BIRD_MAX_Y = SKY_SPACE_START`, so the ceiling is exactly
  where night starts falling; below `SKY_SPACE_START` the sky stays
  day). All flight behaviour is generic over `isBirdKind` (`pigeon`/`parrot`): every former `pigeon*` function/constant is now `bird*` (`BIRD_*`), with old `pigeon*` names kept as `_test` aliases only. 80% of spawns are parrots (`PARROT_FRACTION = 0.8`, `rollBirdKind`, ~40 parrots + 10 pigeons): new `kind: "parrot"` (`makeParrotMesh`, 48 combos in `PARROT_COMBOS` over the 4-colour `PARROT_COLORS` pool red/green/blue/yellow — body/head free, wing base and wing tip each differ from body, head and each other; chest = wing base, tail = wing tips, crest = body, black-over-tan beak; weighted pick (`PARROT_SAME_HEAD = 0.8`): 80% head==body (20% per body colour) + 20% head!=body (5% per body colour), body uniform, wings uniform among allowed; variant stored as `parrotVar`/`userData.parrotVar` and persisted via the mob `look` byte), same `hw 0.25 / h 0.5`, same pivots (`wingL/wingR/body/head`) so `birdAnimate` flaps them unchanged; TNT kills respawn the same kind (`killBird`/`respawnChainMob` preserve kind + variant) and V-chains roll a mixed lead. In the Nether the band is `NETHER_BIRD_MIN_Y` 17 (`NETHER_FIRE_LEVEL` + 5, above the lava lake) to `netherBirdCeiling()` (max crater rim + 8 — crater lava fills to rim level so the rim ring is the max solid height; `NETHER_BIRD_MAX_Y` 300 is only the pre-generation fallback), selected per mob/live dim via `birdDimOf`/`birdBandMin/Max(For)` and used by `bandReturnTarget`/`birdRandomTarget`/`birdReachableTarget`/`birdNewArc`/`spawnSingleBird`/`birdSpotOutOfView`/`respawnChainMob`/`updateBird`. Nether legs fly short and wander: leg picks use 12–30-block targets with 70% arcs (climbing/diving turns), so the flock and its chains constantly change direction instead of riding long straight sky-shots; altitude mixes local ±30 cruise with 25% re-seeds (`pigeonNetherLegY`, full-band for long legs, ±75 window for short ones), spreading birds evenly from the lava floor (17) to the volcano tops. No perching in the Nether — the perch roll/hop in `pigeonNextLeg`/`pigeonTakeoff` are Overworld-gated and strays take off at once — birds only fly the band. No lava contact either: `pigeonLavaAt` rejects any LAVA intersecting the flight box, wired into `pigeonProbeFree`/`pigeonSegmentFree` (Nether-gated, so avoidance steers around crater lakes, cascades and the sea like rock) and into Nether spawns (`spawnSinglePigeon`/`pigeonSpotOutOfView`/`respawnChainMob` via probe) plus carry-release lift-out; coop logic is Overworld-gated (`houseInteriorFor`/`isMobOnRoof` return false outside it). In the End every flying mob shares the dragon's square column (`±END_PLATFORM_R`, `DRAGON_MIN_Y..DRAGON_MAX_Y` = platform +7..+22, via
  ...
  `bandReturnTarget` homing, no per-frame Y snap — only the square XZ clamp and
  ...
  wander targets are clamped into the square and positions hard-clamped after
  physics (outward per-axis velocity stripped, Y capped at `DRAGON_MAX_Y`), with the
  ...
  `vy+1..4` + pyramid roof `vy+5..vy+8` cached per house for 0.25 s, door cells
  ...
  smoothed in `grappleTowPos` at `dt*6`) for as long as the button is held; the spring
  damps against the ride velocity plus turn-rate feedforward (`pm.vel - rideAxV*followDist`,
  same `_chAx`/`_chAxV` math as chain links at `dt*8`/`dt*3`, so the player swings behind
  fast-turning rides instead of cutting the corner — no-op on straight flight) with the cap
  funding the swing (`max(base*2.2, liveVel*1.5, rideSpd + 8 + |turn|*followDist)`); follow
  ...
  highest solid pyramid step per column, up to the `vy+8` apex), cloud tops (`pigeonCloudTopAt`, now with an
  optional y-range) and tree tops (`pigeonTreeTopAt`, highest `LOG`/`LEAVES`
  with headroom) alike — roof portal/obsidian (plus legacy stone/planks) count as perch support only on house
  roofs (`pigeonPerchSupports` checks `houseRoofCell`, `vy+5..vy+8` in shrinking footprint; solid slabs never validate as portal windows since every frame check demands an air interior, so roofs never teleport). Perched birds are spread evenly
...
- **Save/load**: binary format (`SAVE_MAGIC`, version 35) capturing world
  ...
  pair for flowers in v3, the nether dim/seed/blocks added in v4; mob
  ...
  mob yaw is a full `Float32` in v30 (43 bytes/entry; pre-v30 8-bit yaw bytes still
  decode); parrot variant rides the existing `look` byte via `mobLookIndex` (kind code 7 `parrot` added in v31, no size change; pre-v31 saves hold pigeons only and load unchanged); cat robe rides the same `look` byte (kind code 8 `cat` added in v32, no size change; pre-v32 saves top-up cats fresh); restore is exact-place (`placeMobExact`/`mobRestoreOverlapsPlaced`,
  shared by save-load and portal trips): the saved spot is kept bit-for-bit
  (epsilon lift only), `settleMobSpot` runs solely on true world collision, an
  overlap against an already-restored mob nudges minimally at the same height
  instead of re-snapping to the ground, and wander restarts immediately with
  dispersed targets (`restoreInitialTarget`: roof/pen/wolf/village/baby-aware,
  sequential so `mobPenalty` spreads picks; `wanderT=2+rand*4` only desyncs
  arrivals, no forced close `restoreFirstTarget`, no `_settleUntil`) so the
  village never stands still or stampedes into a tangle on load/return;
  adults restored inside their own house resume `inside` (staggered `insideT`,
  `randomInsidePoint` target — no synchronized door exodus), babies keep
  parent-relative targets;
  ...
  remain_at_pause`);
  growable soils (`growableSoils` xyz + timer remain, in v22 as xyz only, v23 adds
  the timer, v27 adds the wet flag + soak remain, v28 adds the moon-water flag (`liq`, so the grey shell survives reload); claims/plant state re-derive live, in-flight `pineGrowths` as
  soil+`m`/`e`/`idx`/`acc` in v26 with v23/v24/v25 growths skipped);
  garland style (1 byte) + finished pines (`plantedPines` xyz + `m`/`e`/`seed`,
  8 bytes each) in v33 (v32 holds cats but no garland section: pre-v33 loads start with no garlands; the style byte is fixed spirale);
  global star-style choice (1 byte) + per-block star styles (`starVariants`, one per dimension, 5 bytes each) in v34 (pre-v34 loads start with choice 0 Yellow and no star styles); moon pine `m` up to 12 accepted on load in v34 (`MOON_PINE_MAX_M`); in v35 the per-block entries are gone (choice byte only; v34 variant entries are skipped on load, pre-v35 loads start with choice 0); moon pine `m` capped at 8 afterwards (saves holding 9..12 skip those pines on load);   leftover id-22 `STAR` blocks from v34 saves load but are never meshed (`BLOCK_INFO` has no entry) and can be broken away;
  ...
  altitude (`placeMobExact` keeps the saved height with a same-height nudge,
  and the post-restore target reset skips `isChained`
  ...
  dimension (`pendingCarriedDim` + `pendingCarriedIdx`,
  appended u8 + Int32, -1 when empty-handed; saves from the Nether/End append the live
  held mob of that dimension so it survives) and restored still in hands (`mode="carried"`,
  ghost transparency, arms visible; chain pairs touching it are skipped);
  per-portal return memory (`netherExit`/`endExit`, in v16, recorded by
  `recordDimExit` on every Nether/End departure so returns land near the portal
  used, always with the portal behind you via `faceAwayFromPortal`);
  the live dragon (`hp`/`hitCount`/position/yaw/pitch, in v17, written only when
  saving in the End mid-fight — portal travel still respawns it fresh, and a
  cleared or dying dragon saves as absent except a mid-animation death (v20
  writes `dying`/`deathIdx`/death anchor while the 1.5 s shake runs, so reload
  resumes the agony and finishes cleared instead of resurrecting); `hitCount`
  widened to 16-bit in v18)
  and all live TNT (placed-fuse
  countdowns with block coords, in-flight homing bombs with an explicit fly
  flag (v18 — the old `by<0` inference broke when `setUint16` clamped -1 to 0,
  collapsing every volley onto one map key), target kind plus
  mob position for re-linking, volley arrival sync (`tntEta` per-target
  remaining time, in v18, so every bomb of a chain-sent volley still lands
  together instead of only the first), and queued blasts with remaining delay; dragon
  targets re-link to the restored `dragon.mob`, mob targets by kind plus
  position (exact first, then nearest within 3.5 blocks so reseated chain
  members still link),   pigeon-burst queue items persist as target-less bursts,
  volley `tntEta` sync is skipped
  and re-establishes naturally);
  live explosion visuals (up to 24 in-flight `bursts`, tagged at spawn with
  center + kind + tint in v19, replayed fresh via the `spawn*` functions on
  load — visual-only, no re-damage — so saving mid-blast still shows the
  explosion when you reload; teleport bursts are tagged but skipped);
  `restoreSave` rebuilds return-portal frames (if missing) before `rebuildMeshes`,
  so cross-dimension `refreshBlocks` can never poison the dim-less chunk cache
  (`chunkMeshes` is keyed `"cx_cz"` with no dimension), then refreshes
  `colTops` per touched dimension and recomputes Nether rivers/volcanoes from
  `netherSeed` when loading a non-empty Nether;
  ...
  cleared   (no mid-agony resume); older v1/v2/v3
  ...
  pre-v31 saves have no parrots stored (kind code 7 `parrot` added in v31,
  topped up mixed with pigeons by `spawnBirds` on load);
  pre-v32 saves have no cats stored (kind code 8 `cat` added in v32,
  topped up 1-per-baby by `spawnVillagers` on load);
  ...
  bytes and restore exactly like v30 otherwise;
  ...
  held, and grabbing/releasing dirties the world, so the hold always reaches disk); every pointer-unlock
  persists first (the `pointerlockchange` save runs before the menu/help guards,
...
- Natural language routing: "spawn/create a new workspace ..." → follow `.opencode/commands/wt-add.md`
  ...
  (rebases, merges to master, deletes the workspace);
