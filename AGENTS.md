...
- **Save/load**: binary format (`SAVE_MAGIC`, version 38) capturing world
  ...
  global star-style choice (1 byte) + per-block star styles (`starVariants`, one per dimension, 5 bytes each) in v34 (pre-v34 loads start with choice 0 Yellow and no star styles); moon pine `m` up to 12 accepted on load in v34 (`MOON_PINE_MAX_M`); in v35 the per-block entries are gone (choice byte only; v34 variant entries are skipped on load, pre-v35 loads start with choice 0); moon pine `m` capped at 8 afterwards (saves holding 9..12 skip those pines on load);   leftover id-22 `STAR` blocks from v34 saves load but are never meshed (`BLOCK_INFO` has no entry) and can be broken away; in v36 every planted pine and in-flight growth carries a `shape` byte plus one global pine-choice byte (both retired since the G choice was removed — always written 1/0, skipped on load, all pines build as cones); in v37 one global decor-visibility byte (B toggle, pre-v37 loads default to on); in v38 one star byte per planted pine (`pineStarOk` at save: at least one of the 4 top spire chunks visible) plus a broken-pine-cell section (8 bytes/entry, validated on load against live pines + `AIR`, orphans dropped — destroyed chunks no longer resurrect; a flag-0 pine with intact crown replays birth on load);
...
- **HUD/UI**: crosshair, hotbar with slot icons (wheel or K/L selects), dimension
  label, pine-style label (`#pine`, shown only as `Decor: off` while B has decorations
  hidden — B toggles garlands + stars live with immediate rebuild),
